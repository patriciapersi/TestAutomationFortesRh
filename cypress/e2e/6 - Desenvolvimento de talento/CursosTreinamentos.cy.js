...
describe('Cursos e Treinamentos', () => {
  ...
  it('Clonar um curso', () => {
    ...
    cy
      ...
      .validaMensagem('Curso clonado com sucesso.')
  })

  it('Clonar curso e compartilhar com outra empresa', () => {
    cy
      .generalButtons('Clonar', dados.curso)
      .get('.p-checkbox-box').eq(1).click()
      .clickNewButton('Clonar')
      .validaMensagem('Curso clonado com sucesso.')

    //Verificando que o curso foi clonado na outra empresa
    cy.reload()
    cy.alteraEmpresa(dados.empresa_nome)
    cy.contains('Continuar').click()
    cy.navigate('/desenvolvimento/cursos')
    cy.contains(dados.curso + ' (Clone)').should('be.exist')
